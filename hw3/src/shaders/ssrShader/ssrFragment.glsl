...
vec3 EvalDiffuse(vec3 wi, vec3 wo, vec2 uv) {
  ...
  float ndotwi = max(0., dot(GetGBufferNormalWorld(uv), wi));
  return rho * INV_PI * ndotwi;
}
...
bool RayMarch(vec3 ori,vec3 dir,out vec3 hitPos){
  ...
  for(int i=0;i<STEP_NUM;++i){
    ...
    if(GetDepth(p)>GetGBufferDepth(uv)){
      hitPos=p;
      return true;
    }
    p+=stepDir;
...
void main(){
  ...
  for(int i=0;i<SAMPLE_NUM;++i){
    ...
    vec3 worldDir=normalize(mat3(normal,b1,b2)*dir);
    
    vec3 hitPos;
    if(RayMarch(vPosWorld.xyz,worldDir,hitPos)){
      vec2 hitUV=GetScreenCoordinate(hitPos);
      L_indir+=EvalDiffuse(wi,-worldDir,hitUV)*EvalDiffuse(worldDir,wo,uv)*EvalDirectionalLight(hitUV)/pdf;
    }
  }
