...
//                  0.0, 1.0);
//   }, 10000000);
const std::vector<double> cosine_lobe = {0.886227, 0.0, 1.02333, 0.0, 0.0, 0.0, 0.495416, 0.0, 0.0};
...
#define CHECK(condition, message)                                                     \
  do {                                                                                \
    if (!(condition)) {                                                               \
      std::cerr << "Check failed (" #condition ") in " << __FILE__ << ":" << __LINE__ \
...
double FastFMod(double x, double m) { return x - (m * floor(x / m)); }
...
void ComputeBandRotation(int l, std::vector<Eigen::MatrixXd>* rotations) {
  ...
  for (int m = -l; m <= l; m++) {
    for (int n = -l; n <= l; n++) {
      ...
      // u, v, w are not zero
      if (!NearByMargin(u, 0.0)) u *= U(m, n, l, *rotations);
      if (!NearByMargin(v, 0.0)) v *= V(m, n, l, *rotations);
      if (!NearByMargin(w, 0.0)) w *= W(m, n, l, *rotations);
...
double ImageYToTheta(int y, int height) { return M_PI * (y + 0.5) / height; }
...
std::unique_ptr<std::vector<double>> ProjectFunction(int order, const SphericalFunction& func,
                                                     int sample_count) {
...
std::unique_ptr<std::vector<double>> ProjectSparseSamples(int order,
  ...
  CHECK(order >= 0, "Order must be at least zero.");
  CHECK(dirs.size() == values.size(), "Directions and values must have the same size.");
  ...
  Eigen::VectorXd soln =
      basis_values.jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(func_values);
...
std::unique_ptr<Rotation> Rotation::Create(int order, const Eigen::Quaterniond& rotation) {
  ...
  CHECK(NearByMargin(rotation.squaredNorm(), 1.0), "Rotation must be normalized.");
...
const Eigen::MatrixXd& Rotation::band_rotation(int l) const { return band_rotations_[l]; }

template <typename T>
void Rotation::Apply(const std::vector<T>& coeff, std::vector<T>* result) const {
...
template double EvalSHSum<double>(int order, const std::vector<double>& coeffs, double phi,
                                  double theta);
template double EvalSHSum<double>(int order, const std::vector<double>& coeffs,
                                  const Eigen::Vector3d& dir);

template float EvalSHSum<float>(int order, const std::vector<float>& coeffs, double phi,
                                double theta);
template float EvalSHSum<float>(int order, const std::vector<float>& coeffs,
                                const Eigen::Vector3d& dir);

template Eigen::Array3f EvalSHSum<Eigen::Array3f>(int order,
...
template <>
void Rotation::Apply<Eigen::Array3f>(const std::vector<Eigen::Array3f>& coeff,
