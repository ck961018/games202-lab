...
namespace ProjEnv {
std::vector<std::unique_ptr<float[]>> LoadCubemapImages(const std::string &cubemapDir, int &width,
...
float CalcArea(const float &u_, const float &v_, const int &width, const int &height) {
  ...
  float angle =
      CalcPreArea(x0, y0) - CalcPreArea(x0, y1) - CalcPreArea(x1, y0) + CalcPreArea(x1, y1);
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        for (int l = 0; l <= SHOrder; ++l) {
          for (int m = -l; m <= l; ++m) {
            double basicFunc = sh::EvalSH(l, m, Vector3d(dir.x(), dir.y(), dir.z()).normalized());
            SHCoeffiecents[sh::GetIndex(l, m)] += Le * basicFunc * area;
          }
        }
      }
    }
 ...
 public:
  ...
  virtual void preprocess(const Scene *scene) override {
    ...
    if (m_Type == Type::Interreflection) {
      ...
      MatrixXf intersectionCoeffs;
      intersectionCoeffs.resize(SHCoeffLength, mesh->getVertexCount());
      intersectionCoeffs.setZero();
      for (int i = 0; i < mesh->getVertexCount(); ++i) {
        const Point3f &v = mesh->getVertexPositions().col(i);
        const Normal3f &n = mesh->getVertexNormals().col(i).normalized();
        std::function<double *(const Point3f, const Normal3f, int)> sumCoeffsFunc =
            [&](const Point3f &curV, const Normal3f &curN, int depth) {
              double res[SHCoeffLength] = {0.0};
              if (depth > m_Bounce) {
                return res;
              }
              ...
              for (int t = 0; t < sample_side; t++) {
                for (int p = 0; p < sample_side; p++) {
                  ...
                  Eigen::Array3d d = sh::ToVector(phi, theta);
                  Vector3f dir = Vector3f(d.x(), d.y(), d.z()).normalized();
                  double H = dir.dot(curN);
                  if (H > 0.0) {
                    Ray3f ray = Ray3f(curV, dir);
                    Intersection intersection;
                    if (scene->rayIntersect(ray, intersection)) {
                      const auto &ids = intersection.tri_index;
                      const auto &normals = intersection.mesh->getVertexNormals();
                      const auto &bary = intersection.bary;

                      Normal3f intersectionNormal =
                          Normal3f(normals.col(ids.x()).normalized() * bary.x() +
                                   normals.col(ids.y()).normalized() * bary.y() +
                                   normals.col(ids.z()).normalized() * bary.z())
                              .normalized();
                      auto nextSumCoeffs =
                          sumCoeffsFunc(intersection.p, intersectionNormal, depth + 1);
                      for (int index = 0; index < SHCoeffLength; ++index) {
                        auto tmp = (m_TransportSHCoeffs.col(ids.x()).coeffRef(index) * bary.x() +
                                    m_TransportSHCoeffs.col(ids.y()).coeffRef(index) * bary.y() +
                                    m_TransportSHCoeffs.col(ids.z()).coeffRef(index) * bary.z() +
                                    nextSumCoeffs[index]) *
                                   H;
                        res[index] += tmp;
                      }
              ...
              double weight = 1 / (sample_side * sample_side);
              for (int index = 0; index < SHCoeffLength; ++index) {
                res[index] *= weight;
              }
              return res;
            };
        auto tmpCoeffs = sumCoeffsFunc(v, n, 1);
        for (int j = 0; j < SHCoeffLength; ++j) {
          intersectionCoeffs.col(i).coeffRef(j) = tmpCoeffs[j];
        }
      }
      m_TransportSHCoeffs += intersectionCoeffs;
    }
  ...
  Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &ray) const {
    ...
    const Eigen::Matrix<Vector3f::Scalar, SHCoeffLength, 1> sh0 = m_TransportSHCoeffs.col(
                                                                its.tri_index.x()),
                                                            sh1 = m_TransportSHCoeffs.col(
                                                                its.tri_index.y()),
                                                            sh2 = m_TransportSHCoeffs.col(
                                                                its.tri_index.z());
    ...
    // TODO: you need to delete the following four line codes after finishing your calculation to
    // SH,
